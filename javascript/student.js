...
function updateStudentDashboard(data) {
    ...
    // Removed to allow static page titles in the header
    ...
    // Show/hide semester selector based on academic level (JHS has no semesters)
    const semWrapper = document.getElementById('semesterSelectorWrapper');
    if (semWrapper) {
        if (data.level && data.level.toLowerCase().includes('junior')) {
    ...
    // Update navbar profile badge name
    ...
    if (navSub) {
        navSub.textContent = data.level === 'junior-high' ? 'Junior High School' : 'Senior High School';
    }
